  ...
  % \begin{align*}
    ...
    \begin{table}[h]
      ...
      \cline{1-3}
       ...
       \texttt{(a)-[FriendOf]->(b)-[FriendOf]->(c),} \\ \texttt{(b)-[Knows]->(d)}& \texttt{Two Hops And Branch} & $\widetilde\pi_{m} \Bigg(  \rename {f_{id}} m F \NJoin \rename {p_{id}} m F  \NJoin \rename {p_{id}} m K \Bigg)$ &  &  \\ \cline{1-3}
       ...
       \texttt{(a)-[FriendOf]->(b)-[FriendOf]->(c),} \\ \texttt{(c)-[FriendOf]->(d),}  \texttt{(b)-[Knows]->(d)}& \texttt{Three Hops And Branch} & $ \widetilde\pi_{m,n} \Bigg (  \rename{f_{id}}{m}{F} \NJoin \rename{p_{id}, f_{id}}{m,n}{F}   \NJoin \rename{p_{id}}{n}{F} \NJoin \rename{p_{id}}{m}{K} \Bigg ) $ &  &  \\ \cline{1-3}
      \end{tabular}%
      }
      \end{table}
